...
import datasets
from MDFormer import MambaDynamicFormer, CustomMidBlock
# custom imports
...
def load_custom_model(checkpoint_dir):
    ...
    mid_channels = model.mid_block.resnets[0].norm1.weight.shape[0]
    mdformer = MambaDynamicFormer(in_channels=mid_channels, d_state=16, expand=2, kernel_size=3)
    model.mid_block = CustomMidBlock(model.mid_block, mdformer)

    model.load_state_dict(load_file(os.path.join(checkpoint_dir, 'diffusion_pytorch_model.safetensors')))
...
def main(
    ...
    if (mode == "train" and resume_epoch is not None) or "eval" in mode:
        ...
        model = load_custom_model(os.path.join(config.output_dir, 'unet'))
